...
import {
  ...
  AlertCircle, Plus, Presentation, Check
} from 'lucide-react';
...
export default function PdfToPowerpointTool() {
  ...
  const [slideLayout, setSlideLayout] = useState('LAYOUT_16x9'); // 'LAYOUT_16x9' | 'LAYOUT_4x3'
  ...
  // Results: Array of { originalName, pptxBlob, blobUrl, size }
  ...
  /* ── Core PDF to PPTX Slide Generator with pptxgenjs ──── */
  const convertPdfToPptx = async (item, onProgressUpdate) => {
    ...
    for (let pNum = 1; pNum <= numPages; pNum++) {
      onProgressUpdate?.(`Rendering slide ${pNum} of ${numPages}...`);
      ...
      const slideImageData = canvas.toDataURL('image/jpeg', 0.88);

      // Extract text content for slide notes / text layer
      const textContent = await page.getTextContent();
      const pageText = textContent.items.map(it => it.str).join(' ').trim();

      const slide = pptx.addSlide();
      
      // Place page image as full-bleed slide background / visual
      slide.addImage({
        data: slideImageData,
        x: 0,
        y: 0,
      ...
      // Add speaker notes with the raw text from that page
      if (pageText) {
  ...
  const handleConvertAll = async () => {
    ...
    setProgress(15);
    setProgressText('Initializing presentation generator...');
    setErrorMsg('');
    ...
    try {
      ...
      for (let i = 0; i < total; i++) {
        ...
        setProgress(pct);
        setProgressText(`Converting ${item.name} to PowerPoint slides...`);

        const res = await convertPdfToPptx(item, (txt) => setProgressText(txt));
        results.push(res);
      }

      setProgress(95);
      setProgressText('Packaging PPTX presentation files...');

      if (results.length > 1) {
        const zip = new JSZip();
        results.forEach(r => {
          const pptxName = r.originalName.replace(/\.pdf$/i, '.pptx');
          zip.file(pptxName, r.pptxBlob);
        });
      ...
      analytics.trackToolExecution('pdf-to-powerpoint', true, { count: results.length });

      // Auto Download single file
      if (results.length === 1) {
        const link = document.createElement('a');
        link.href = results[0].blobUrl;
        link.download = results[0].originalName.replace(/\.pdf$/i, '.pptx');
      ...
      console.error(err);
      setErrorMsg('Failed to convert PDF to PowerPoint slides.');
      setStatus('idle');
    }
  };
    ...
    <div className="w-full max-w-7xl mx-auto space-y-6 font-sans">
      ...
      {status === 'idle' && files.length === 0 && (
        <div
          ...
          className={`relative border-2 border-dashed rounded-3xl p-10 sm:p-16 text-center transition-all flex flex-col items-center justify-center min-h-[360px] cursor-pointer ${
            isDraggingOver
              ...
              : 'border-zinc-300 dark:border-[#2A2E45] bg-[#F8FAFC]/60 dark:bg-[#141622]/60 hover:border-orange-400 dark:hover:border-orange-600'
          }`}
        ...
        >
          ...
          <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-3xl bg-orange-600 text-white flex items-center justify-center shadow-xl shadow-orange-600/25 mb-6 group-hover:scale-105 transition-transform">
            <Presentation className="w-10 h-10 sm:w-12 sm:h-12" />
          </div>
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-4 text-[11px] text-zinc-400 dark:text-zinc-500 font-medium">
            ...
            <span>•</span>
            <span>Every Page Becomes a PowerPoint Slide</span>
            <span>•</span>
            <span>Native PPTX Presentation Output</span>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start animate-fade-in">
          ...
          <div className="lg:col-span-8 space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              ...
              <div
                onClick={() => fileInputRef.current?.click()}
                className="border-2 border-dashed border-zinc-300 dark:border-[#2A2E45] hover:border-orange-400 rounded-2xl p-4 flex flex-col items-center justify-center text-center cursor-pointer min-h-[160px] bg-zinc-50/50 dark:bg-[#141622]/40 transition-colors"
              >
                <div className="w-10 h-10 rounded-full bg-orange-50 dark:bg-orange-950/50 text-orange-600 flex items-center justify-center mb-2">
            ...
            <div className="p-4 sm:p-6 rounded-3xl bg-white dark:bg-[#141622] border border-zinc-200 dark:border-[#2A2E45] shadow-sm space-y-5">
              ...
              <div className="space-y-1 text-left">
                ...
                <p className="text-[11px] text-zinc-500">
                  Transform PDF pages into editable PowerPoint presentation slides (.pptx).
                </p>
              </div>
              ...
              <div className="space-y-2.5">
                ...
                >
                  ...
                  <p className="text-[11px] text-zinc-500 dark:text-zinc-400">
                    Standard widescreen presentation layout for modern screens and projectors.
                  </p>
                </div>
                ...
                >
                  ...
                  <p className="text-[11px] text-zinc-500 dark:text-zinc-400">
                    Traditional 4:3 aspect ratio matching classic slide dimensions.
                  </p>
                </div>
              </div>
      ...
      {status === 'processing' && (
        <div className="rounded-3xl bg-white dark:bg-[#141622] border border-zinc-200 dark:border-[#2A2E45] p-10 sm:p-16 text-center space-y-6 shadow-sm">
          <div className="relative w-20 h-20 mx-auto">
            <div className="w-20 h-20 rounded-full border-4 border-orange-100 dark:border-orange-950 border-t-orange-600 animate-spin" />
          ...
          <div className="space-y-2 max-w-md mx-auto">
            <h3 className="text-base sm:text-lg font-black text-zinc-900 dark:text-white">
              Converting PDF to PowerPoint PPTX...
            </h3>
            <p className="text-xs text-zinc-500 dark:text-zinc-400 font-medium">
              {progressText || 'Generating slides and visual layouts...'}
            </p>
          </div>
        ...
        <div className="rounded-3xl bg-white dark:bg-[#141622] border border-zinc-200 dark:border-[#2A2E45] p-8 sm:p-14 text-center space-y-6 shadow-sm animate-scale-up">
          ...
          <div className="space-y-2 max-w-lg mx-auto">
            <h2 className="text-xl sm:text-2xl font-black text-zinc-900 dark:text-white">
              PDF has been converted to editable POWERPOINT!
            </h2>
            <p className="text-xs sm:text-sm text-zinc-500 dark:text-zinc-400">
              Your presentation deck is ready to open and present in Microsoft PowerPoint, Google Slides, or Apple Keynote.
            </p>
          </div>
          ...
          <div className="inline-flex flex-wrap items-center justify-center gap-3 px-4 py-2 rounded-xl bg-zinc-100 dark:bg-[#1B1E2E] text-xs font-semibold text-zinc-700 dark:text-zinc-300">
            <span className="truncate max-w-xs">{convertedResults[0].originalName.replace(/\.pdf$/i, '.pptx')}</span>
            <span>•</span>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 max-w-md mx-auto pt-2">
            {convertedResults.length === 1 ? (
              <a
                ...
                download={convertedResults[0].originalName.replace(/\.pdf$/i, '.pptx')}
              ...
              <a
                href={downloadZipUrl}
                download="converted_powerpoint_decks.zip"
                className="w-full sm:w-auto flex-1 px-8 py-4 rounded-2xl bg-red-600 hover:bg-red-700 active:scale-95 text-white font-black text-base shadow-xl shadow-red-600/30 transition-all flex items-center justify-center gap-2.5 cursor-pointer"
              >
                <Download className="w-5 h-5" />
