...
import { ShieldCheck, Download, RotateCcw, AlertCircle, FileText, Sparkles, Eye, File, Lock, Key } from 'lucide-react';
...
export default function ProtectPdfTool() {
  ...
  const handleProtect = async () => {
    ...
    setProgress(20);
    setProgressText('Loading document contents...');
    setErrorMsg('');
    ...
    try {
      setProgress(50);
      setProgressText('Applying strong encryption algorithms...');

      const arrayBuffer = await file.arrayBuffer();
      const encryptedBytes = await encryptPDF(new Uint8Array(arrayBuffer), password);

      setProgress(85);
      setProgressText('Finalizing secured file stream...');

      const blob = new Blob([encryptedBytes], { type: 'application/pdf' });
      const url = URL.createObjectURL(blob);

      setResultBlobUrl(url);
      setResultFilename(file.name.replace(/\.[^/.]+$/, '') + '_protected.pdf');
      setResultSize(blob.size);
      setProgress(100);
      setStatus('completed');
    } catch (err) {
      console.error(err);
      setErrorMsg('Failed to encrypt PDF file. Please try again.');
      setStatus('ready');
      setProgress(0);
    }
  };
      ...
      <div className="rounded-3xl overflow-hidden bg-white dark:bg-[#141622] border border-blue-200 dark:border-[#2A2E45] shadow-xl p-6 sm:p-8">
        {errorMsg && (
          <div className="mb-4 flex items-start gap-3 p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800">
            ...
            <div className="flex-1 text-left">
              <p className="text-xs font-bold text-slate-800 dark:text-slate-200">Protection Error</p>
        ...
        {status === 'idle' && (
          ...
          >
            ...
            <h3 className="text-lg font-bold text-zinc-950 dark:text-white mb-2">
              Select PDF file to password protect
            </h3>
        ...
        {status === 'ready' && file && (
          <div className="space-y-6 text-left animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 rounded-2xl bg-zinc-50 border border-zinc-200">
              <div className="space-y-4">
                <h4 className="text-sm font-bold text-zinc-950 dark:text-white flex items-center gap-1.5">
                  <Key className="w-4 h-4 text-purple-600" />
                  Set PDF Password
                </h4>
                <p className="text-xs text-zinc-500">Choose a secure password to restrict access to this document. Unauthorised users will not be able to view its contents.</p>
              </div>

              <div className="space-y-4">
                <div className="space-y-1.5">
                  <label className="block text-xs font-bold text-zinc-700">Type Password:</label>
                  <input
                    type="password"
                    value={password}
                    onChange={e => setPassword(e.target.value)}
                    placeholder="Enter document password"
                    className="w-full text-xs sm:text-sm px-3.5 py-2.5 rounded-xl border border-zinc-300 bg-white font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-purple-500"
                  />
                </div>

                <div className="space-y-1.5">
                  <label className="block text-xs font-bold text-zinc-700">Repeat Password:</label>
                  <input
                    type="password"
        ...
        {status === 'processing' && (
          <div className="py-12 text-center space-y-6">
            ...
            <div className="space-y-1">
              <h4 className="text-sm font-bold text-zinc-900 dark:text-white">Encrypting PDF Document</h4>
        ...
        {status === 'completed' && (
          <div className="py-8 text-center space-y-6 animate-scale-in">
            ...
            <div className="space-y-1">
              <h3 className="text-xl font-bold text-zinc-950 dark:text-white">PDF Encrypted Successfully!</h3>
              <p className="text-xs text-zinc-500">Your PDF file has been protected and is ready for download.</p>
            </div>
            ...
            <div className="max-w-md mx-auto p-4 rounded-2xl bg-zinc-50 border border-zinc-200 flex items-center justify-between text-left">
              ...
              <span className="text-[10px] font-bold px-2 py-0.5 bg-purple-100 text-purple-700 rounded-md border border-purple-200">
                Encrypted
              </span>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 max-w-md mx-auto">
              <button
                ...
                onClick={(e) => {
                  ...
                  link.download = resultFilename || 'protected_document.pdf';
            ...
            <p className="text-[10px] text-zinc-400 mt-2 font-medium">
              🔒 In-browser encryption: Your password never leaves your browser.
            </p>
          </div>
        )}
      </div>
