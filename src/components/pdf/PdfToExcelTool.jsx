...
import {
  ...
  AlertCircle, Plus, Table, Check, Sheet
} from 'lucide-react';
...
export default function PdfToExcelTool() {
  ...
  const [sheetLayout, setSheetLayout] = useState('one_sheet'); // 'one_sheet' | 'multi_sheets'
  ...
  // Results: Array of { originalName, xlsxBlob, blobUrl, size }
  ...
  /* ── Core PDF to XLSX Table Extractor ─────────────────── */
  const convertPdfToXlsx = async (item, onProgressUpdate) => {
    ...
    for (let pNum = 1; pNum <= numPages; pNum++) {
      onProgressUpdate?.(`Extracting tabular rows from page ${pNum} of ${numPages}...`);
      ...
      if (sheetLayout === 'one_sheet' && numPages > 1) {
        pageRows.push([`=== Page ${pNum} ===`]);
      }

      for (const y of sortedY) {
        // Sort items left to right across columns
        const items = lineMap.get(y).sort((a, b) => a.x - b.x);

        // Detect column separation by large X gap
        const rowCols = [];
        let curCol = [];
        let prevX = -1;

        for (const item of items) {
          if (prevX !== -1 && item.x - prevX > 35) {
            rowCols.push(curCol.join(' ').trim());
            curCol = [item.text];
          } else {
            curCol.push(item.text);
          }
          prevX = item.x;
        }
        if (curCol.length > 0) rowCols.push(curCol.join(' ').trim());

        if (rowCols.length > 0) pageRows.push(rowCols);
      }

      if (sheetLayout === 'multi_sheets') {
        const worksheet = XLSX.utils.aoa_to_sheet(pageRows.length ? pageRows : [['No text found']]);
        XLSX.utils.book_append_sheet(workbook, worksheet, `Page ${pNum}`);
      } else {
        combinedRows.push(...pageRows);
      }
    }

    if (sheetLayout === 'one_sheet') {
      const worksheet = XLSX.utils.aoa_to_sheet(combinedRows.length ? combinedRows : [['No text found']]);
    ...
    return {
      ...
      xlsxBlob,
      blobUrl: URL.createObjectURL(xlsxBlob),
      size: xlsxBlob.size
    };
  };
  ...
  const handleConvertAll = async () => {
    ...
    setProgress(15);
    setProgressText('Extracting table cells and numbers...');
    setErrorMsg('');
    ...
    try {
      ...
      for (let i = 0; i < total; i++) {
        ...
        setProgress(pct);
        setProgressText(`Converting ${item.name} to Excel workbook...`);

        const res = await convertPdfToXlsx(item, (txt) => setProgressText(txt));
        results.push(res);
      }

      setProgress(95);
      setProgressText('Finalizing Excel spreadsheets...');

      if (results.length > 1) {
        const zip = new JSZip();
        results.forEach(r => {
          const xlsxName = r.originalName.replace(/\.pdf$/i, '.xlsx');
          zip.file(xlsxName, r.xlsxBlob);
        });
      ...
      analytics.trackToolExecution('pdf-to-excel', true, { count: results.length });

      // Auto Download single file
      if (results.length === 1) {
        const link = document.createElement('a');
        link.href = results[0].blobUrl;
        link.download = results[0].originalName.replace(/\.pdf$/i, '.xlsx');
      ...
      console.error(err);
      setErrorMsg('Failed to convert PDF to Excel spreadsheet.');
      setStatus('idle');
    }
  };
    ...
    <div className="w-full max-w-7xl mx-auto space-y-6 font-sans">
      ...
      {status === 'idle' && files.length === 0 && (
        <div
          ...
          className={`relative border-2 border-dashed rounded-3xl p-10 sm:p-16 text-center transition-all flex flex-col items-center justify-center min-h-[360px] cursor-pointer ${
            isDraggingOver
              ...
              : 'border-zinc-300 dark:border-[#2A2E45] bg-[#F8FAFC]/60 dark:bg-[#141622]/60 hover:border-emerald-400 dark:hover:border-emerald-600'
          }`}
        ...
        >
          ...
          <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-3xl bg-emerald-600 text-white flex items-center justify-center shadow-xl shadow-emerald-600/25 mb-6 group-hover:scale-105 transition-transform">
            <Table className="w-10 h-10 sm:w-12 sm:h-12" />
          </div>
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-4 text-[11px] text-zinc-400 dark:text-zinc-500 font-medium">
            ...
            <span>•</span>
            <span>Auto-Detects Table Columns &amp; Rows</span>
            <span>•</span>
            <span>Native XLSX Spreadsheet Output</span>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start animate-fade-in">
          ...
          <div className="lg:col-span-8 space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              ...
              <div
                onClick={() => fileInputRef.current?.click()}
                className="border-2 border-dashed border-zinc-300 dark:border-[#2A2E45] hover:border-emerald-400 rounded-2xl p-4 flex flex-col items-center justify-center text-center cursor-pointer min-h-[160px] bg-zinc-50/50 dark:bg-[#141622]/40 transition-colors"
              >
                <div className="w-10 h-10 rounded-full bg-emerald-50 dark:bg-emerald-950/50 text-emerald-600 flex items-center justify-center mb-2">
            ...
            <div className="p-4 sm:p-6 rounded-3xl bg-white dark:bg-[#141622] border border-zinc-200 dark:border-[#2A2E45] shadow-sm space-y-5">
              ...
              <div className="space-y-1 text-left">
                ...
                <p className="text-[11px] text-zinc-500">
                  Convert PDF invoices, tables, and financial statements to Excel spreadsheets (.xlsx).
                </p>
              </div>
              ...
              <div className="space-y-2.5">
                ...
                >
                  ...
                  <p className="text-[11px] text-zinc-500 dark:text-zinc-400">
                    Merges all extracted tables and pages into a single continuous sheet.
                  </p>
                </div>

                <div
                  onClick={() => setSheetLayout('multi_sheets')}
                  className={`p-3.5 rounded-2xl border-2 transition-all cursor-pointer text-left ${
                    sheetLayout === 'multi_sheets'
                      ? 'border-emerald-500 bg-emerald-50/40 dark:bg-emerald-950/20'
                      : 'border-zinc-200 dark:border-[#2A2E45] hover:border-zinc-300'
                  }`}
                >
                  ...
                  <p className="text-[11px] text-zinc-500 dark:text-zinc-400">
                    Creates a separate Excel sheet tab for each PDF page.
                  </p>
                </div>
              </div>
      ...
      {status === 'processing' && (
        <div className="rounded-3xl bg-white dark:bg-[#141622] border border-zinc-200 dark:border-[#2A2E45] p-10 sm:p-16 text-center space-y-6 shadow-sm">
          <div className="relative w-20 h-20 mx-auto">
            <div className="w-20 h-20 rounded-full border-4 border-emerald-100 dark:border-emerald-950 border-t-emerald-600 animate-spin" />
          ...
          <div className="space-y-2 max-w-md mx-auto">
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400 font-medium">
              {progressText || 'Extracting table cells and formulas...'}
            </p>
          </div>
        ...
        <div className="rounded-3xl bg-white dark:bg-[#141622] border border-zinc-200 dark:border-[#2A2E45] p-8 sm:p-14 text-center space-y-6 shadow-sm animate-scale-up">
          ...
          <div className="space-y-2 max-w-lg mx-auto">
            <h2 className="text-xl sm:text-2xl font-black text-zinc-900 dark:text-white">
              PDF has been converted to editable EXCEL!
            </h2>
            <p className="text-xs sm:text-sm text-zinc-500 dark:text-zinc-400">
              Your spreadsheet is ready to open and edit in Microsoft Excel, Google Sheets, or Apple Numbers.
            </p>
          </div>
          ...
          <div className="inline-flex flex-wrap items-center justify-center gap-3 px-4 py-2 rounded-xl bg-zinc-100 dark:bg-[#1B1E2E] text-xs font-semibold text-zinc-700 dark:text-zinc-300">
            <span className="truncate max-w-xs">{convertedResults[0].originalName.replace(/\.pdf$/i, '.xlsx')}</span>
            <span>•</span>
