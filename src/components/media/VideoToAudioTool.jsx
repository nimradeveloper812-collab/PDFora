...
import {
  UploadCloud, Music, Download, RotateCcw, AlertCircle,
...
export default function VideoToAudioTool() {
  ...
  const handleConvert = async () => {
    ...
    setProgress(10);
    setProgressText('Initializing FFmpeg audio extraction...');
    setErrorMsg('');
    ...
    try {
      ...
      setErrorMsg(err.message || 'Failed to extract audio track. Please try another video file.');
      setStatus('ready');
    }
  };
      ...
      {status === 'idle' && (
        ...
        >
          ...
          <p className="text-zinc-500 max-w-md mx-auto mb-6 text-sm">
            Drag & drop your video file here, or click to browse. Supports MP4, WebM, MKV, AVI, MOV, WMV, FLV, and TS.
          </p>
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-zinc-100 text-xs font-semibold text-zinc-600">
            ...
            Max file size: 200 MB • 100% Private & Cleaned up
          </div>
        </div>
      )}
        ...
        <div className="bg-white rounded-3xl border border-zinc-200 shadow-sm p-6 sm:p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {SUPPORTED_AUDIO_OUTPUTS.map((fmt) => (
                  <button
                    key={fmt.id}
                    type="button"
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {[
                  { id: '320k', label: '320 kbps', badge: 'Ultra' },
                  { id: '256k', label: '256 kbps', badge: 'High' },
                  { id: '192k', label: '192 kbps', badge: 'Recommended' },
                  { id: '128k', label: '128 kbps', badge: 'Balanced' },
                  { id: '96k', label: '96 kbps', badge: 'Compact' },
                ].map((b) => (
                  <button
                    key={b.id}
                    type="button"
                    onClick={() => setBitrate(b.id)}
                    disabled={status === 'processing' || targetFormat === 'wav' || targetFormat === 'flac'}
                    className={`p-3 rounded-xl border text-left transition ${
                      bitrate === b.id
                        ? 'border-blue-600 bg-blue-50/60 ring-2 ring-blue-600/20'
                        : 'border-zinc-200 hover:border-zinc-300 bg-white'
                    } ${targetFormat === 'wav' || targetFormat === 'flac' ? 'opacity-40 cursor-not-allowed' : ''}`}
                  >
                    <p className={`font-bold text-sm ${bitrate === b.id ? 'text-blue-700' : 'text-zinc-800'}`}>
                      {b.label}
                    </p>
                    <p className="text-[11px] text-zinc-500 truncate">{b.badge}</p>
                  </button>
                ))}
              </div>
              {(targetFormat === 'wav' || targetFormat === 'flac') && (
                <p className="text-xs text-zinc-500 mt-2">
                  * Lossless formats preserve exact studio PCM/FLAC depth without lossy bitrate restrictions.
                </p>
              )}
            </div>
          </div>
          ...
          {status === 'processing' ? (
            <div className="p-6 rounded-2xl bg-blue-50/60 border border-blue-100 text-center">
              ...
              <p className="font-bold text-zinc-900 mb-1">{progressText || 'Extracting Audio Track...'}</p>
            ...
            >
              ...
              Extract {targetFormat.toUpperCase()} Audio
      ...
      {status === 'completed' && resultBlob && (
        <div className="bg-white rounded-3xl border border-zinc-200 shadow-sm p-6 sm:p-8 text-center animate-fadeIn">
          ...
          <p className="text-zinc-600 text-sm mb-6">
            Your {targetFormat.toUpperCase()} audio file is ready for download.
          </p>
          ...
          <div className="max-w-md mx-auto grid grid-cols-2 gap-4 p-4 rounded-2xl bg-zinc-50 border border-zinc-200 mb-6 text-left">
            ...
            <div>
              <p className="text-xs text-zinc-500 font-medium">Extracted Audio ({targetFormat.toUpperCase()})</p>
